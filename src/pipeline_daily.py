...
import schedule
import subprocess
from db import run_start, run_finish

# Load environment
CRON_TIME = os.getenv("CRON_TIME", "03:10")  # default 3:10 AM
PYTHON_PATH = os.getenv("PYTHON_PATH", "python3")
SCRAPER_SCRIPT = os.path.join(os.path.dirname(__file__), "happyruh_scraper.py")
...
def run_pipeline():
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            run_finish(run_id, "error", None, None, result.stderr or result.stdout)
            return

        print("✅ Pipeline completed successfully")
        print(result.stdout[-500:])  # tail log

        run_finish(run_id, "ok", None, None, None)
    except Exception as e:
        run_finish(run_id, "error", None, None, str(e))
